...
internal sealed interface ChatView {
    ...
    data class ThreadsLoaded(val threads: List<dev.egograph.shared.dto.Thread>) : ChatView
    data class ThreadsLoadFailed(val error: String) : ChatView

    data class ThreadSelected(val thread: dev.egograph.shared.dto.Thread) : ChatView
    data object ThreadSelectionCleared : ChatView

    data object MessagesLoadingStarted : ChatView
    data class MessagesLoaded(val messages: List<dev.egograph.shared.dto.ThreadMessage>) : ChatView
    data class MessagesLoadFailed(val error: String) : ChatView
    ...
    data class ModelsLoadFailed(val error: String) : ChatView
...
internal class ChatStoreFactory(
    ...
    fun create(name: String = "ChatStore"): ChatStore {
        val store = storeFactory.create<ChatIntent, Unit, ChatView, ChatState, ChatLabel>(
            name = name,
...
private object ChatReducerImpl :
    ...
    override fun ChatState.reduce(msg: ChatView): ChatState = when (msg) {
        is ChatView.ThreadsLoadingStarted -> copy(
            isLoadingThreads = true,
            threadsError = null
        )

        is ChatView.ThreadsLoaded -> copy(
            threads = msg.threads,
            isLoadingThreads = false,
            threadsError = null
        )

        is ChatView.ThreadsLoadFailed -> copy(
            isLoadingThreads = false,
        ...
        is ChatView.MessagesLoadingStarted -> copy(
            isLoadingMessages = true,
            messagesError = null
        )

        is ChatView.MessagesLoaded -> copy(
            messages = msg.messages,
            isLoadingMessages = false,
            messagesError = null
        )

        is ChatView.MessagesLoadFailed -> copy(
            isLoadingMessages = false,
            messagesError = msg.error
        )

        is ChatView.ModelsLoadingStarted -> copy(
            isLoadingModels = true,
            modelsError = null
        )

        is ChatView.ModelsLoaded -> copy(
            models = msg.models,
        ...
        is ChatView.ModelsLoadFailed -> copy(
            isLoadingModels = false,
