...
- コマンド文字列を `|`, `;`, `&&`, `||` で分割し、各セグメントの先頭コマンドを照合
- クォート内の文字列は照合対象外
...
| ツール | チェック関数 |
|---|---|
| `bash` | `check_command_paths()` — コマンド文字列内のパスを検査 |
| `read` / `write` / `edit` | `check_path()` — ファイルパスを検査 |
| `grep` / `find` / `ls` | `check_path()` — 検索パスを検査 |
...
各ツールにリスクレベルを割り当て、High リスクツールは Web UI からの実行時にオペレーターの明示的な承認を要求する。
...
Web UI からの High リスクツール実行時:

1. `__approved: true` フラグの有無を確認
2. なし → エラーを返し、オペレーターの承認を要求
3. あり → 実行

TUI・CLI・チャットからの実行は承認なしで可能。
