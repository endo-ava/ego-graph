...
4. [環境変数オーバーライド](#4-環境変数オーバーライド)
5. [プロバイダープリセット](#5-プロバイダープリセット)
6. [デフォルトパス](#6-デフォルトパス)
7. [セットアップウィザード](#7-セットアップウィザード)
8. [WebUI 設定画面](#8-webui-設定画面)
9. [CLI ランタイムコマンド](#9-cli-ランタイムコマンド)
...
- **エージェントファースト**: API キーなどの秘匿値は YAML に平文保存するが、外部 API レスポンスでは `has_api_key` の真偽値のみ返却し、値そのものはマスクする。
...
| `api_key` | `string \| null` | 条件付き | API 認証キー。`localhost` 系プロバイダーでは不要。秘匿フィールド |
...
| `auth_token` | `string` | 条件付き | なし | Web 有効時は必須。ブラウザアクセス時の認証トークン。秘匿フィールド |
...
| `bot_token` | `string` | 条件付き | なし | Discord Bot トークン。有効時は必須。環境変数 `EGOPULSE_DISCORD_BOT_TOKEN` でも指定可能。秘匿フィールド |
| `provider` | `string \| null` | 任意 | `null` | このチャネル専用のプロバイダーオーバーライド |
| `model` | `string \| null` | 任意 | `null` | このチャネル専用のモデルオーバーライド |
| `allowed_channels` | `[u64]` | 任意 | `[]` | Bot が反応するチャンネル ID。空の場合は全チャンネルで反応 |
...
| `bot_token` | `string` | 条件付き | なし | Telegram Bot トークン。有効時は必須。環境変数 `EGOPULSE_TELEGRAM_BOT_TOKEN` でも指定可能。秘匿フィールド |
| `bot_username` | `string` | 条件付き | なし | Bot のユーザー名。グループ内で `@botname` メンション検知に使用。有効時は必須 |
| `provider` | `string \| null` | 任意 | `null` | このチャネル専用のプロバイダーオーバーライド |
| `model` | `string \| null` | 任意 | `null` | このチャネル専用のモデルオーバーライド |
| `allowed_user_ids` | `[i64]` | 任意 | `[]` | 許可するユーザー ID。空の場合は全ユーザーを許可 |
...
| 環境変数 | 対象フィールド | 型 |
|---|---|---|
| `EGOPULSE_LOG_LEVEL` | `log_level` | `string` |
| `EGOPULSE_WEB_HOST` | `channels.web.host` | `string` |
| `EGOPULSE_WEB_PORT` | `channels.web.port` | `u16` |
| `EGOPULSE_WEB_ENABLED` | `channels.web.enabled` | `bool` |
| `EGOPULSE_WEB_AUTH_TOKEN` | `channels.web.auth_token` | `string` |
| `EGOPULSE_WEB_ALLOWED_ORIGINS` | `channels.web.allowed_origins` | `string`（カンマ区切り） |
| `EGOPULSE_DISCORD_BOT_TOKEN` | `channels.discord.bot_token` | `string` |
| `EGOPULSE_TELEGRAM_BOT_TOKEN` | `channels.telegram.bot_token` | `string` |
...
| `channels.telegram.allowed_user_ids` | 非表示 |
...
TUI / CLI チャットセッション内で使用できる設定コマンド。設定変更は即座にセッションに反映され、YAML ファイルにも永続化される。

### 9.1 コマンド一覧

| コマンド | 引数 | 説明 |
|---|---|---|
| `/providers` | なし | 全プロバイダー一覧を表示 |
| `/provider` | `[name] [--scope web\|discord\|telegram\|global]` | プロバイダーを切り替え。`--scope` でチャネルを指定。省略時は `global` |
| `/provider reset` | `--scope <channel>` | チャネルのプロバイダー設定をデフォルトにリセット |
| `/models` | なし | 現在のプロバイダーで利用可能なモデル一覧を表示 |
| `/model` | `[name] [--scope web\|discord\|telegram\|global]` | モデルを切り替え。`--scope` でチャネルを指定。省略時は `global` |
| `/model reset` | `--scope <channel>` | チャネルのモデル設定をプロバイダーデフォルトにリセット |

### 9.2 スコープの動作

- `global` を指定した場合、`config.default_provider` / `config.default_model` が更新される
- チャネル名を指定した場合、そのチャネルの `provider` / `model` オーバーライドが更新される
- `reset` はオーバーライドを `null` に戻す
...
| フィールド | YAML（手動） | Setup Wizard | WebUI | CLI (`/provider` `/model`) |
|---|:---:|:---:|:---:|:---:|
| **グローバル** | | | | |
...
| `channels.telegram.allowed_user_ids` | R/W | なし | なし | なし |
...
| **CLI コマンド** | チャット中の動的切替 | セッション中に即座に反映。スコープ単位で柔軟に切替 | プロバイダー/モデル切替のみ。システム設定は対象外 |
