...
4. アプリ情報を入力：
   ...
   - **Redirect URI**: `http://localhost:8888/callback` （重要：正確に入力）
...
5. リダイレクトされたURL（`http://localhost:8888/callback?code=...`）全体をコピー
6. ターミナルに貼り付けてEnter
...
```
https://accounts.spotify.com/authorize?client_id=CLIENT_ID&response_type=code&redirect_uri=http://localhost:8888/callback&scope=user-read-recently-played%20playlist-read-private%20playlist-read-collaborative
```
