...
- Plexus 側 runtime の push token / device metadata
...
| Plexus runtime の push token / device metadata | SQLite | 必要なら DuckDB へ複製 | runtime 側の運用メタデータ |
...
terminal/runtime 系メタデータは、**EgoGraph ではなく Plexus 側で SQLite を正本**とする。
...
- データ量が小さい
- 更新中心であり分析中心ではない
- EgoGraph ではなく runtime repository の責務である
...
1. runtime 系の正本は Plexus 側 SQLite に統一する
