...
設定ファイルは任意のパスに配置し、systemd unit の `ExecStart` で `--config` で指定する。
以下の例では `/etc/egopulse/` を使用する。

```bash
sudo mkdir -p /etc/egopulse
sudo nano /etc/egopulse/egopulse.config.yaml
```
...
```bash
# 設定ファイルの確認
cat /etc/egopulse/egopulse.config.yaml

# 手動起動でエラー確認
egopulse --config /etc/egopulse/egopulse.config.yaml start
...
| エラー | 原因 | 解決策 |
|--------|------|--------|
| `config file not found` | `--config` パスの誤り | `ls /etc/egopulse/egopulse.config.yaml` で確認 |
