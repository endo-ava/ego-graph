...
> このドキュメントは旧バージョン（React + Capacitor）のデプロイ手順です。
> 最新の手順は `frontend-android.md` を参照してください。
> 旧アプリ本体はモノレポから分離され、`endo-ava/egograph-frontend-capacitor-legacy` で保守されています。
...
- Legacy repo: <https://github.com/endo-ava/egograph-frontend-capacitor-legacy>
